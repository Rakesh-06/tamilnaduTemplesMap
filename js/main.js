...
var map, templeInfowindow; //globally declaring the main variables......
...
var TempleViewModel = function() {
  ...
  self.filterTemple = ko.computed(function() {
    ...
    if (!filter) {
      ...
      templesData.forEach(function(templeItem) {
      return ko.utils.arrayFilter(self.templesData(), function(templeItem) {
        ...
        if (templeCase.indexOf(filter) >= 0) {
            return templeItem.markers.setVisible(true);
          }else{
            return templeItem.markers.setVisible(false);
          }
        });
      });
      }

...
var map, marker, templeInfowindow; //globally declaring the main variables......
...
function initMap() {
    ...
    map = new google.maps.Map(document.getElementById('map'), {
        center: {
            lat: 9.952168,
            lng: 78.125244
        }, // this center lat-lng not the exact center, but the location of my home "hah...hah..haaaa!!!"
        zoom: 16,
        styles: styles
    });
    ...
    for (var i = 0; i < templesData.length; i++) {
        ...
        templesData[i].eachTempleMarker = marker;
...
function populateInfoWindow(marker, templeInfowindow) {
    ...
    $.ajax(wikiUrl, {
        ...
        if (templeInfowindow.marker != marker) {
            ...
            templeInfowindow.setContent('<div>' + marker.title + '<br> <a href="' + url + '">Click here! Know the HOLY LAND</a></div>');
            ...
            function getStreetView(data, status) {
                if (status == google.maps.StreetViewStatus.OK) {
                    ...
                    templeInfowindow.setContent('<div>' + marker.title + '<br> <a href="' + url + '">Click here! Know the HOLY LAND</a></div><div id="pano"></div>');
                    var panoramaOptions = {
                    ...
                    templeInfowindow.setContent('<div>' + marker.title + '<br> <a href="' + url + '">Click here! Know the HOLY LAND</a></div><div>Sorry FRIEND...... No Street View Found!</div>');
                }
            }
...
function toggleBounce(marker) {
    if (marker.getAnimation() !== null) {
        ...
        setTimeout(function() {
            marker.setAnimation(null);
        }, 1000); // set timeout for the boune to occur for 1sec
    }
}
...
var TempleViewModel = function() {
    ...
    self.clickOver = function(templesData) { //click func() for the marker
        ...
        populateInfoWindow(templesData.eachTempleMarker, templeInfowindow);
    };
    ...
    self.filterTemple = ko.computed(function() {
        ...
        var TempleArray = ko.utils.arrayFilter(self.templesData(), function(templeItem) {
            var templeCase = templeItem.title.toLowerCase(),
                filterCase = self.filterInput().toLowerCase(),
                visibleTemple = templeItem.eachTempleMarker;
            if (templeCase.indexOf(filterCase) >= 0) {
                if (visibleTemple) {
                    visibleTemple.setVisible(true);
                }
                return true;
            } else {
                if (visibleTemple) {
                    visibleTemple.setVisible(false);
                }
                return false;
            }
        });
